...
%%
program: 
			function_list						{ node_init ( $$ = malloc(sizeof(node_t)), program_n, NULL, 1, $1); };
function_list: 	
			function 							{ node_init ( $$ = malloc(sizeof(node_t)), function_list_n, NULL, 1, $1); }
...
variable_list: 
			variable							{ node_init ( $$ = malloc(sizeof(node_t)), variable_list_n, NULL, 1, $1); }
...
argument_list: 
			expression_list						{ node_init ( $$ = malloc(sizeof(node_t)), argument_list_n, NULL, 1, $1); }
			| 									{ $$ = NULL; };
parameter_list: 
			variable_list						{ node_init ( $$ = malloc(sizeof(node_t)), parameter_list_n, NULL, 1, $1); }
			|									{ $$ = NULL; } ;
declaration_list: 
			declaration_list declaration 		{ node_init ( $$ = malloc(sizeof(node_t)), declaration_list_n, NULL, 2, $1, $2); }
			| 									{ $$ = NULL; };
function: 
			FUNC variable '(' parameter_list ')' statement	{ node_init ( $$ = malloc(sizeof(node_t)), function_n, NULL, 3, $2, $4, $6); };
statement: 
			assignment_statement			 	{ node_init ( $$ = malloc(sizeof(node_t)), statement_n, NULL, 1, $1); }
...
block: 
			'{' declaration_list statement_list '}'	{ node_init ( $$ = malloc(sizeof(node_t)), block_n, NULL, 2, $2, $3); }

assignment_statement: 
			variable ASSIGN expression 		 	{ node_init ( $$ = malloc(sizeof(node_t)), statement_n, NULL, 2, $1, $3); }
			| variable '[' expression ']' ASSIGN expression		{ node_init ( $$ = malloc(sizeof(node_t)), statement_n, NULL, 1, $1); };
return_statement: 
			RETURN expression					{ node_init ( $$ = malloc(sizeof(node_t)), statement_n, NULL, 1, $2); };
print_statement:
			PRINT print_list					{ node_init( $$ = malloc(sizeof(node_t)), statement_n, NULL, 1, $2); };
null_statement: 
			CONTINUE						 	{ node_init ( $$ = malloc(sizeof(node_t)), statement_n, NULL, 1, $1); };
if_statement: 
...
while_statement: 
			WHILE expression DO statement DONE				 	{ node_init ( $$ = malloc(sizeof(node_t)), while_statement_n, NULL, 2, $2, $4); };
expression: 
			expression '+' expression 				{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 3, $1, "+", $3); }
			| expression '-' expression 			{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 3, $1, "-", $3); }
			| expression '*' expression 			{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 3, $1, "*", $3); }
			| expression '/' expression 			{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 3, $1, "/", $3); }
			| '-' expression 						{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 2, "-", $2); }
			| expression POWER expression 			{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, NULL, 2, $1, "^", $3); }
			...
			| variable 								{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, yytext, 1, $1); }
			| variable '(' argument_list ')' 		{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, "F", 2, $1, $3); }
			| variable '[' expression ']'			{ node_init ( $$ = malloc(sizeof(node_t)), expression_n, "A", 2, $1, $3); };
declaration: 
			VAR variable_list						{ node_init ( $$ = malloc(sizeof(node_t)), declaration_n, NULL, 1, $2); };
variable: 
			IDENTIFIER								{ node_init ( $$ = malloc(sizeof(node_t)), variable_n, NULL, 1, $1); };

indexed_variable: 
			variable '[' integer ']'				{ node_init ( $$ = malloc(sizeof(node_t)), variable_n, NULL, 2, $1, $3); };
integer: 
			NUMBER									{ node_init ( $$ = malloc(sizeof(node_t)), integer_n, NULL, 1, $1); };
print_item: 
			expression							 	{ node_init ( $$ = malloc(sizeof(node_t)), print_item_n, NULL, 1, $1); }
			| text									{ node_init ( $$ = malloc(sizeof(node_t)), text_n, NULL, 1, $1); };
text: 
			STRING									{ node_init ( $$ = malloc(sizeof(node_t)), text_n, yytext, 1, $1); };
%%
